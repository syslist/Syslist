...
EndServiceSetup:
  ...
  ifFileExists "$EXEDIR\config" ConfigInstall
...
ConfigInstall:
  ClearErrors
  ExecWait '"$INSTDIR\SCAConf.exe" /install /Config "$EXEDIR\config $CMDLINE"'
  ifErrors ConfigFailed
  Return
...
Function .onInstSuccess
  ...
  ifFileExists "$EXEDIR\config" FinalConfigInstall
