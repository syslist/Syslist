...
public:
	...
	virtual long ChooseSystemProxy (char * URIHostOnly, long *URIPort)
	{
		...
		if (m_UsingProxy == false)
			return HTTPTransport::ChooseSystemProxy(URIHostOnly, URIPort);
	...
	long SendProxyHeader() 
	{
		...
		//if (m_FinalDocPort != m_DefaultPort) {
			...
			PortString << m_FinalDocPort;
			
			Status = WriteString(":");
			Status = WriteString(PortString.str().c_str());
		//}
		...
		long ServerCode = 0;
		sscanf(ServerResponse, "%*s %ld", &ServerCode);
		if (ServerCode != 200)
			return ERROR_GEN_FAILURE;
	...
	long OpenURI(const char * URIString)
	{
		long Status;
		...
		if (err != 1) {
			SSLError = SSL_get_error(m_SSL, err);
			return ERROR_GEN_FAILURE;
		}
